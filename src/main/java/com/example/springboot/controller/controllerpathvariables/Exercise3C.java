...
public class Exercise3C {
    ...
    public ResponseEntity<?> getMealByDescription(@PathVariable String phrase){
        ...
        for (Meal m:meals) {
            if(m.getDescription().equals(phrase)){
                mealThatMarches = m;
            }
        }

        if(mealThatMarches != null){
            return ResponseEntity.status(HttpStatus.FOUND).body(mealThatMarches);
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No Meal found that has the description given");
        }
    }
}
