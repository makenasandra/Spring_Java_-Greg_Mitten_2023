...
public class Exercise2 {
    ...
    public ResponseEntity<?> getMealByName(@PathVariable String name){
        Meal mealThatMatchesName = null;
        for (Meal m: meals) {
           if(name.equals(m.getName())){
               mealThatMatchesName = m;
           }
        }

        if(mealThatMatchesName != null) {
            return ResponseEntity.status(HttpStatus.FOUND).body(mealThatMatchesName);
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No Meal found that has the name given");
        }
    }
}
